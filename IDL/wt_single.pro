...
cgwindow, woxmargin = [50, 50], woymargin = [50, 50], wxsize = 720, wysize = 720
...
; common
axes = { xrange: [time_fft[0]-dt2, time_fft[nt-1]+dt2], $
           yrange: [omg_ar[0], omg_ar[ n_elements(omg_ar)-1]], $
           xtitle: 't x10!e'+string(sf,'(I1)')+'!N'+c_we0m, $
           ytitle: var.omgc+' '+c_we0, $
           xstyle: 1, ystyle: 1, xminor: 1, ticklen: -0.02, charsize: 2}

;axes=axes_a
;axes=Create_struct(axes, 'xtickformat',"(A1)")
cgImage,wt_f[*,*,4], /axes, minvalue=plmin, maxvalue=plmax, axkeywords=axes, /window
...
cgColorbar, /vertical, /right, $
  range=[plmin, plmax], $
  pos=[xwindow[1]+0.01, ywindow[0], xwindow[1]+0.02, ywindow[1]],  $
  charsize=2, title=bartitle, /addcmd

; Annotation
;cgText, 0.45, 0.01, 't     x10!e'+string(sf,'(I1)')+'!N '+ c_we0m, /normal, charsize=2, /addcmd


;mssim = ssim(wt_f[*,*,2],wt_f[*,*,3])
