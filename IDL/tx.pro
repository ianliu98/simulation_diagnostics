...
;  ntime = ntime/2
  ...
  for jt = 0L, ntime-1 do begin
    ...
    if ( (jjt MOD tskip) eq 0 ) then begin
	  ...
	  forfy_rcd[*, jnt] = divf[*,0]
	  forfz_rcd[*, jnt] = divf[*,1]
...
if freqdis eq 1 then begin
  ...
  if sep_mode eq 'separation' then begin  
    ...
    for jx=0, rnx-1 do begin
      ...
      ;bacf[*, jx] = fre_cal( trans_bacf[*,jx], trans_bacf1[*,jx], dt )
    endfor
    forf = Transpose(forf[*,*])
    bacf = Transpose(bacf[*,*])
;    if elim eq 1 then begin
...
while (1 GT 0) DO BEGIN
  ...
  IF (sep_mode eq 'separation') then begin   
    ...
    cgImage, bacf[*,*], /axes,  $
      ...
      top=topd, wmulti=[0,2,1], /window
    
    if (while_tmp eq 0) then begin
      xyaxes.ytitle = ''
  ...
  cgColorbar, /vertical, /right,  $
    ...
    position=[!x.window[1]+0.02, !y.window[0], !x.window[1]+0.03, !y.window[1]], /addcmd
