...
if (jf eq 5) then begin
 ...
 ; cgwindow, woxmargin = [10,10], woymargin = [10,10], wxsize = 1080, wysize=1080
  cgImage, abs(forf), /axes, AXKEYWORDS=axisset, $
    minvalue=plmin, maxvalue=plmax, top=topd, $
    xrange = [min(x_ax_f), max(x_ax_f)], yrange=[min(tf),max(tf)]/10000.0, $
    xtitle='h [c' +var.lomgc+ '!S!De0!R!U-1!N]', title=var.omgc +'=' +string(minf[jf], '(f4.2)') +'-' +string(maxf[jf], '(f4.2)') +'  ' +c_we0, $
...
endelse  
  ...
  cgColorbar, /vertical, /right,  $
    ...
    position=[0.9, 0.3, 0.92, 0.7], charsize=3, /addcmd
