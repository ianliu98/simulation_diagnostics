...
;    Window:  A string containing the name of the smoothing window to return.  
;        Options are 'boxcar', 'gaussian', 'hanning', 'triangle'.  The default 
;        is a boxcar window.
;
; KEYWORD PARAMETERS:
;    BOXCAR:  Sets the output to a boxcar window.  This is the default.  If 
;        set to a value, it replaces Width (obsolete option).
;    DIMENSION:  The dimension of the filter, of type integer.  The default is 
;        1.
;    TRIANGLE:  Sets the output to a triangle window.  The default is a boxcar 
;        boxcar window.  If set to a value, it replaces Width (obsolete option).
;
; OUTPUTS:
...
;    Written by:  Daithi A. Stone (dastone@runbox.com), 2000-06-28.
;    Modified:  DAS, 2001-01-29 (fixed odd width bug).
;    Modified:  DAS, 2003-11-18 (added Window input, DIMENSION keyword,  edited 
;        style)
