...
; he,hz
omg_max 	= 0.1
;fft_pnt		= 4096L    ;modified later
fft_pnt		= 512L    ;modified later
;tskip = 1
tskip = 4
fc = 13000 ; [Hz]
tslide = 16 
...
for jx=0, px-1 do begin
  ...
  if (jx eq 0) then begin
    cgImage, wt[*,*, jx], /axes, minvalue=plmin, maxvalue=plmax, interpolate=interp, axkeywords=axes, multimargin=mmargin, wmulti=[0,5,3], /window
  endif else begin
    cgImage, wt[*,*, jx], /axes, minvalue=plmin, maxvalue=plmax, interpolate=interp, axkeywords=axes, multimargin=mmargin, /addcmd
  endelse 
  if (ocont eq 1) then begin
    ncontour = 3
    contourLevels =  cgConLevels( wt[*,*, jx], NLevels=ncontour, MinValue=plmax-1, MaxValue=plmax )
    cgContour, wt[*,*, jx], /OnImage, label=0, Levels=contourLevels, Thick=0.01, /addcmd
  endif
...
; unit of h
cgText, xwindow_t+0.02, ywindow_t+0.01, charsize=1,  $
...
for jx=0, px-1 do begin
  ...
  cgImage, wt_f[*,*, jx], /axes, minvalue=plmin, maxvalue=plmax, interpolate=interp,axkeywords=axes, multimargin=mmargin, /addcmd
  if (ocont eq 1) then begin
    ncontour = 3
    contourLevels =  cgConLevels( wt_f[*,*, jx], NLevels=ncontour, MinValue=plmax-1, MaxValue=plmax )
    cgContour, wt_f[*,*, jx], /OnImage, Color='white', label=0, Levels=contourLevels, Thick=0.01, /addcmd
  endif
endfor
...
for jx=0, px-1 do begin
  ...
  cgImage, wt_b[*,*, jx], /axes, minvalue=plmin, maxvalue=plmax, interpolate=interp,axkeywords=axes, multimargin=mmargin, /addcmd
  if (ocont eq 1) then begin
    ncontour = 3
    contourLevels =  cgConLevels( wt_b[*,*, jx], NLevels=ncontour, MinValue=plmax-1, MaxValue=plmax )
    cgContour, wt_b[*,*, jx], /OnImage, Color='white', label=0, Levels=contourLevels, Thick=0.01, /addcmd
  endif
endfor
xwindow = !X.window * xsize  &  ywindow = !Y.window * ysize
xwindow_nw2 = (xwindow[1]-xsize/2.0)/(xsize/2.0) * (xsize/2.0-xmargin2[1]) + xsize/2.0
...
cgColorbar, /vertical, /right, $
  range=[plmin, plmax], $
  pos=[xwindow_b + 0.01, ywindow_b, xwindow_b + 0.02, ywindow_t],  $
  charsize=1.5, title=bartitle, /addcmd

; Annotation
cgText, 0.45, 0.03, 't     x10!e'+string(sf,'(I1)')+'!N '+ c_we0m, /normal, charsize=1, /addcmd
cgText, 0.04, 0.45, var.omgc +' ' + c_we0, orientation=90, /normal, charsize=1, /addcmd
